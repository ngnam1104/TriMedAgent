    ...
    def chat_report(self,message: str, ref_record: str, force_generate=False):
        if force_generate:   # for specific medical suggestion generation task, normally set to false
            ...
            if ret==None:
                ...
                response = self.chat_with_gpt(chat_message)
                # SỬA TẠI ĐÂY: Đóng gói kết quả RAG vào cấu trúc Agent
                thoughts = f"I have successfully retrieved specialized medical knowledge regarding '{query}' from the Merck Manual to provide an accurate assessment."
                # Actions để trống [] vì lúc này Tool đã thực thi xong và trả về kết quả cho LLM
                message = f"Thoughts: {thoughts}\nActions: []\nValue: {response}\n\nNote: Relevant information is sourced from the Merck Manual Professional Edition. ({needed_site})"
